...
const SizeChart = ({ productSizeChart }) => {
  const getColumnHeaders = () => {
    const headers = [
      ...
      {
        key: "product_BR/r_inch",
        label: "BR/r",
        img: BR,
        subheaders: ["inch", "mm"],
        keys: ["product_BR/r_inch", "product_BR/r_mm"],
      },
      ...
      {
        key: "product_vacuum_hg_@_70°F/27°C_mm",
    ...
    return headers.filter((header) =>
      productSizeChart.some((detail) =>
        header.keys
          ? header.keys.some((key) => detail[key])
          : detail[header.key]
      )
    );
  };
        ...
        <thead>
          ...
          <tr>
            {singleHeaders.map((header, index) => (
              <th
                key={index}
                className="px-4 py-2 border bg-newgold text-offwhite"
              >
                --
              </th>
            ))}
            {mainHeaders.flatMap((header) =>
        ...
        <tbody>
          {productSizeChart
            ?.filter((details) =>
              headers.some((header) =>
                header.keys
                  ? header.keys.some((key) => details[key])
                  : details[header.key]
              )
            )
            .map((details, index) => (
              <tr key={index} className="text-[10px] font-medium">
                {singleHeaders.map((header, headerIndex) => (
                  <td
                    key={headerIndex}
                    className="px-4 py-2 text-center border border-newgold"
                  >
                    {details[header.key] || "--"}
                  </td>
                ))}
